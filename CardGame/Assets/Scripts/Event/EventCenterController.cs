...
{
	private static EventCenterController _instance;

	public static EventCenterController Instance ()
	{
		if (null == _instance)
			_instance = new EventCenterController ();
		return _instance;
	}
						...
						if (oneFun.Target != null && oneFun.Method != null) {
							if (oneFun.Target  is MonoBehaviour) {
								...
								if (mon != null) {
									...
									if (obj != null) {									
										obj.SendMessage (oneFun.Method.Name, null, SendMessageOptions.DontRequireReceiver);
									} else {
